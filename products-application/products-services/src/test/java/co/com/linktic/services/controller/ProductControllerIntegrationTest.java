...
@TestPropertySource(properties = "security.api-key=secret-key-products-asdfg")
class ProductControllerIntegrationTest {
	...
	void testCreateProduct() throws Exception {
		...
		var result = mockMvc
				.perform(post("/api/v1/product").contentType(MediaType.APPLICATION_JSON).content(body)
						.header("x-api-key", "secret-key-products-asdfg"))
	...
	void testFindById() throws Exception {
		...
		var createResult = mockMvc.perform(post("/api/v1/product").contentType(MediaType.APPLICATION_JSON)
				.content(requestBody).header("x-api-key", "secret-key-products-asdfg")).andExpect(status().isOk())
				.andReturn();

		var json = objectMapper.readTree(createResult.getResponse().getContentAsString());
		var id = json.get("data").get("id").asText();

		mockMvc.perform(get("/api/v1/product/" + id).header("x-api-key", "secret-key-products-asdfg"))
	...
	void testFindById_NotFound() throws Exception {
		mockMvc.perform(get("/api/v1/product/99999").header("x-api-key", "secret-key-products-asdfg"))
	...
	void testPagination() throws Exception {
		for (var i = 0; i < 5; i++) {
			...
			mockMvc.perform(post("/api/v1/product").contentType(MediaType.APPLICATION_JSON)
					.header("x-api-key", "secret-key-products-asdfg").content(objectMapper.writeValueAsString(dto)))
			.andExpect(status().isOk());
		}

		mockMvc.perform(get("/api/v1/product?page=0&size=3").header("x-api-key", "secret-key-products-asdfg"))
